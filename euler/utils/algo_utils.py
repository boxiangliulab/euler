...
def read_var_map(opt, reads, variants, somatic_variants):
    ...
    allele_confidence_count = collections.defaultdict(int)
    ...
    for read in reads:
        ...
        for var in mapped_variants:
            ...
            allele, quals = file_utils.get_base_pair_by_var_pos(opt, read, var.end)
            ...
            alleles.append((var.unique_id+':'+str(geno), times, tuple(quals)))
        ...
        for allele, _, quals in allele_list:
            ...
            allele_confidence_count[allele] += sum(map(lambda x: x-file_utils.QUAL_THRESHOLD+1, quals))

        for i in range(0, len(allele_list)-1):
            for j in range(i+1, len(allele_list)):
                (allele_1, times_1, qual_1), (allele_2, times_2, qual_2) = allele_list[i], allele_list[j]
                ...
                vars_.add(allele_list[i][0])
                vars_.add(allele_list[j][0])
    ...
    return allele_linkage_map, edge_barcode_map, len(vars_), allele_linkage_read_count_map, allele_read_count, variant_allele_map, allele_confidence_count
